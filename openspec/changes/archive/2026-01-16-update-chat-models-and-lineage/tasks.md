...
- [x] 5.3 Add integration test for `/api/chat/session/{session_id}/message/{message_id}/graph` endpoint
- [x] 5.4 Manual verification: send chat request, verify graph_data is persisted, retrieve via new endpoint

## 6. Validation
- [x] 6.1 Run `pytest tests/unit/storage -k chat_artifact -v`
- [x] 6.2 Run `pytest tests/unit/llm -k model_routing -v`
- [x] 6.3 Run `pytest tests/integration/api -k chat_graph -v`
- [x] 6.4 Manually test chat endpoints with new routing and verify fallback behavior
