...
class ProjectCreate(BaseModel):
    ...
    id: Optional[str] = Field(None, description="Optional custom project ID (generated if not provided)")
...
class RepositoryResponse(BaseModel):
    ...
    node_count: int = Field(0, description="Number of lineage nodes from this repository")
    last_synced: Optional[str] = Field(None, description="Last sync timestamp (ISO 8601)")
    created_at: Optional[str] = Field(None, description="Creation timestamp (ISO 8601)")
...
class LinkEvidence(BaseModel):
    ...
    reference_type: str = Field(..., description="Type of reference (e.g., 'table_reference', 'import')")
...
class LinkResponse(BaseModel):
    ...
    confidence: Optional[float] = Field(None, ge=0.0, le=1.0, description="Confidence score (for auto-detected)")
    evidence: Optional[list[LinkEvidence]] = Field(None, description="Evidence array (for auto-detected)")
    created_at: Optional[str] = Field(None, description="Creation timestamp (ISO 8601)")
...
class ProjectDetailResponse(BaseModel):
    ...
    links: list[LinkResponse] = Field(..., description="List of links between repositories")


class LinkDetectResponse(BaseModel):
    """Response model for link detection results."""

    detected_links: list[LinkResponse] = Field(..., description="List of detected links")
