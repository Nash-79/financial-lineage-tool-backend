...
@pytest.fixture
def mock_stores():
    with patch("src.api.routers.files.project_store") as mock_proj, patch(
        "src.api.routers.files.repository_store"
    ) as mock_repo, patch("src.api.routers.files.config") as mock_config, patch(
        "src.api.routers.files.get_tracker"
    ) as mock_tracker, patch(
        "src.api.routers.files.artifact_manager"
    ) as mock_artifacts, patch(
        "src.api.routers.files.get_app_state"
    ) as mock_state, patch(
        "src.api.routers.files.open", mock_open()
    ) as mock_file_open, patch(
        "src.api.routers.files.Path.mkdir"
    ) as mock_mkdir:

        # Default Config
        mock_config.UPLOAD_BASE_DIR = "/tmp/uploads"
        mock_config.UPLOAD_MAX_FILE_SIZE_MB = 10
        mock_config.ALLOWED_FILE_EXTENSIONS = [
            ".sql",
            ".json",
            ".ddl",
            ".csv",
        ...
        mock_proj.get.return_value = {"id": "proj_123", "name": "Test Project"}
        ...
        mock_repo.create = AsyncMock(
            return_value={"id": "new_repo_id", "project_id": "proj_123"}
        )
        mock_repo.update_counts = AsyncMock()

        tracker = MagicMock()
        tracker.start_session = AsyncMock(
            return_value=MagicMock(ingestion_id="ingest_123")
        )
        tracker.update_file_status = AsyncMock()
        tracker.file_complete = AsyncMock()
        tracker.file_error = AsyncMock()
        tracker.file_skipped = AsyncMock()
        tracker.complete_session = AsyncMock()
        tracker.log_debug = AsyncMock()
        mock_tracker.return_value = tracker

        run_context = MagicMock()
        run_context.run_id = "run_123"
        run_context.run_dir = "/tmp/run_123"
        run_context.get_artifact_dir.return_value = Path("/tmp/raw_source")
        mock_artifacts.create_run = AsyncMock(return_value=run_context)
        mock_artifacts.register_file = AsyncMock(
            return_value={
                "file_hash": "hash",
                "status": "registered",
        ...
        mock_artifacts.mark_file_processed = AsyncMock()
        mock_artifacts.complete_run = AsyncMock()
        ...
        yield mock_proj, mock_repo, mock_config, mock_state, tracker


@pytest.mark.asyncio
async def test_upload_existing_repo_success(mock_stores):
    mock_proj, mock_repo, _, _, tracker = mock_stores
    ...
    assert res["ingestion_id"] == "ingest_123"
...
async def test_upload_new_repo_success(mock_stores):
    mock_proj, mock_repo, _, _, _ = mock_stores

    files_to_upload = [("files", ("test.json", b"{}", "application/json"))]
    data = {"project_id": "proj_123", "repository_name": "New Repo"}
...
async def test_invalid_file_extension(mock_stores):
    ...
    )  # Endpoint returns 200 with error details in body
    res = response.json()
    assert res["ingestion_id"] == "ingest_123"
...
async def test_file_too_large(mock_stores):
    mock_proj, mock_repo, mock_config, _, _ = mock_stores
