...
@pytest.fixture
def mock_stores():
    with patch("src.api.routers.github.project_store") as mock_proj, patch(
        "src.api.routers.github.repository_store"
    ) as mock_repo:
        ...
        mock_repo.create = AsyncMock(
            return_value={"id": "new_repo_id", "file_count": 0}
        )
...
async def test_github_callback_success(mock_config):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, patch(
        ...
        response = client.post(
            "/api/v1/github/callback", json={"code": "test_code", "state": state}
        )
...
async def test_get_user(mock_github_client_dependency):
    mock_github_client_dependency.get.return_value.json.return_value = {
        "login": "testuser"
    }

    response = client.get(
        "/api/v1/github/user", headers={"Authorization": "Bearer test_token"}
    )
...
async def test_list_repos(mock_github_client_dependency):
    ...
    response = client.get(
        "/api/v1/github/repos", headers={"Authorization": "Bearer test_token"}
    )
...
async def test_get_contents(mock_github_client_dependency):
    ...
    response = client.get(
        "/api/v1/github/contents?repo=user/repo",
...
@pytest.mark.asyncio
async def test_ingest_files_project_not_found(
    mock_stores, mock_github_client_dependency
):
